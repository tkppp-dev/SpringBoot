    ...
    <div class="row justify-content-center">
      <div class="register-form">
        ...
        <form @submit.prevent="submitForm">
          ...
          <div class="field-error" v-if="validationChecker.emailAddress.invalid">
  ...
  updateUsernameValidation(username){
    ...
    if(username.length > 0){
      ...
      if(username.match(/[^a-zA-Z0-9가-힣ㄱ-ㅎ]/g) != null){
        this.username.errors.push('사용자 이름은 한글, 영문자, 숫자로만 이루어져야 합니다.')
      }
...
export default {
  ...
  methods: {
    async submitForm() {
      try {
        if(!this.isFormValid){
          throw new Error('필수 입력 사항중 입력하지 않은 사항이 존재합니다.')
        }
